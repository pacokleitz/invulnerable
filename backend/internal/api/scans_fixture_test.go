...
func TestFilterVulnerabilitiesByOnlyFixed_Mixed(t *testing.T) {
	grypeResult := loadGrypeFixture(t, "grype-output-mixed.json")

	// Test filtering with onlyFixed=true
	filtered := filterMatchesByOnlyFixed(grypeResult.Matches, true)
...
func TestFilterVulnerabilitiesByOnlyFixed_AllFixed(t *testing.T) {
...
func TestFilterVulnerabilitiesByOnlyFixed_AllUnfixed(t *testing.T) {
...
func TestFilterVulnerabilitiesByOnlyFixed_Disabled(t *testing.T) {
	grypeResult := loadGrypeFixture(t, "grype-output-mixed.json")

	// Test with onlyFixed=false (disabled filtering)
	filtered := filterMatchesByOnlyFixed(grypeResult.Matches, false)

	// Should include all CVEs when filtering is disabled
	assert.Len(t, filtered, len(grypeResult.Matches), "should not filter when onlyFixed=false")
...
func TestSeverityCounts_WithFixtures(t *testing.T) {
	...
	fixedMatches := filterMatchesByOnlyFixed(grypeResult.Matches, true)
...
func filterMatchesByOnlyFixed(matches []models.GrypeMatch, onlyFixed bool) []models.GrypeMatch {
	if !onlyFixed {
		return matches
	}
